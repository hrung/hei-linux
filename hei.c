...
__declspec(dllexport) int WPLCSetOSLoad(HEIDevice *pDevice, int Val)
	...
	Buffer[DataOffset+1] = (BYTE)(~FUN_SET_OS_LOAD);
...
__declspec(dllexport) int WPLCReboot(HEIDevice *pDevice)
	...
	Buffer[DataOffset+1] = (BYTE)(~FUN_REBOOT);
...
BOOL EZSendPacket(HEIDevice *pDevice, PGM_BUF *pBuffer, BYTE *Error_message, unsigned short int Len)
{
	...
	switch (Error)
    {
        ...
        {
            strncpy((char*)Error_message, "Message size too large", 81);
            return FALSE;
        }
       
        case HEIE_ZERO_BYTES_RECEIVED:
        {
            strncpy((char*)Error_message, "Attempted to send zero-length message", 81);
            return FALSE;
        }
        ...
        case -1:
        {
            ...
            strncpy((char*)Error_message, "HEIWriteEZethernetPgmSpace call failed with -1", 81);
            return FALSE;
        }
        ...
        {
             //printf("Error %X returned from  HEIWriteEZethernetPgmSpace!\n", Error);
             strncpy((char*)Error_message, "HEIWriteEZethernetPgmSpace call failed with unknown error", 81);        
             return FALSE;
        }
